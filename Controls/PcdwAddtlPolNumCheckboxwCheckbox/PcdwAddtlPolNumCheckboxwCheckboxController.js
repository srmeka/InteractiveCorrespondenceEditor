app.controller('PcdwAddtlPolNumCheckboxwCheckboxController', function ($scope, shareData, $http, HomeService) {
    try {
        ...
        $scope.selectPolicy = function (item) {
            ...
            if (item.checked) {
                $scope.SelectedPolicy.push(item);
            }
            else {
                $scope.SelectedPolicy.splice($scope.SelectedPolicy.indexOf(item), 1);
            }
        }
    ...
    $scope.$on('PcdwAddtlPolNumCheckboxwCheckbox', function (event) {
        try {
