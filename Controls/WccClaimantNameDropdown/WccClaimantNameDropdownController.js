app.controller('WccClaimantNameDropdownController', function ($scope, $http, shareData, HomeService) {
    try {
    ...
    $scope.$on('WccClaimantNameDropdown', function (event) {
        try {
            var outputName = '';
            ...
            if ($scope.selectedName) {
                ...
                if (shareData.shareOutputXML.getElementsByTagName("CLMT_NAME")[0]) {
                    shareData.shareOutputXML.getElementsByTagName("CLMT_NAME")[0].nodeTypedValue = outputName.trim();
                }
                else {
