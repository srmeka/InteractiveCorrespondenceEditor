app.controller('GcAddrOneandTwowGreetingNameController', function ($scope, $http, shareData, HomeService) {
    try {
        ...
        $scope.selectAddr = function () {
            ...
            if ($scope.selectedAddressee) {
                ...
                $scope.greetingName = $scope.selectedAddressee.LastName || $scope.selectedAddressee.Name;
            }
        }
        
        function getGreetingName() {
            HomeService.LookupValue('Greeting').then(function (response) {
                $scope.greetingData = response.data
            }, function (error) {
                $scope.error = error;
            })
        };
    ...
    $scope.$on('GcAddrOneandTwowGreetingName', function (event) {
        try {
            ...
            if ($scope.greetingType && $scope.greetingType.lookupItemCode != 'None') {
                grtg_typlst = $scope.greetingType.lookupItemCode;
            }
            if ($scope.greetingName) {
                grtg_val = $scope.greetingName;
            ...
            HomeService.createPrimaryXML("GRTG_TYPLST", grtg_typlst.trim());
            HomeService.createPrimaryXML("GRTG_VAL", grtg_val.trim());
