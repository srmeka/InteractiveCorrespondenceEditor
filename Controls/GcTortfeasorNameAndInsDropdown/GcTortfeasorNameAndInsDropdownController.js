app.controller('GcTortfeasorNameAndInsDropdownController', function ($scope, $http, shareData, HomeService) {
    try {
    ...
    $scope.$on('GcTortfeasorNameAndInsDropdown', function (event) {
        try {
            ...
            if ($scope.selectedInsurer) {
                ...
                if (outputInsuranceName) {
                    outputInsuranceName = outputInsuranceName.replace(/undefined/g, '');
                }

            }

            HomeService.createPrimaryXML("TORTFEASOR_NM", outputName.trim());
            HomeService.createPrimaryXML("TORTFEASOR_INS_CAR", outputInsuranceName.trim());
