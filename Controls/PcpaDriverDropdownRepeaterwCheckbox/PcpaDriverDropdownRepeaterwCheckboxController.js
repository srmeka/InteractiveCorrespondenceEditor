app.controller('PcpaDriverDropdownRepeaterwCheckboxController', function ($scope, $http, shareData, HomeService) {
    try {
    ...
    $scope.$on('PcpaDriverDropdownRepeaterwCheckbox', function (event) {
        try {
            for (var index in $scope.TotalCount) {
                if ($scope.SelectedDriver[index]) {
                    ...
                    if (selectedData || $scope.freeTextGroup[index]) {
                        ...
                        HomeService.createSecondaryXMLValue("POL_DRVR_DESC", "PROOF_COV_IND", "Y", id);
