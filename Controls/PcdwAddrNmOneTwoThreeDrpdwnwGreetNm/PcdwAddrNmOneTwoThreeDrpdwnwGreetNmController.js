app.controller('PcdwAddrNmOneTwoThreeDrpdwnwGreetNmController', function ($scope, shareData, $http, HomeService) {
    try {
        ...
        $scope.updateGreetingName = function () {
            //updateGreeting(SelectedAddressee, AddresseeName2Chk, SelectedAddressee3);
            $scope.GreetingName = 'Dear ';
            if ($scope.selectedAddressee) {
                if ($scope.selectedAddressee.PartyTypeCd === 'Person') {
                    $scope.GreetingName += $scope.selectedAddressee.Person.PersonName.FirstGivenNm + ' ';

                    if ($scope.selectedAddressee.Person.PersonName.SecondGivenNameInitial)
                        $scope.GreetingName += $scope.selectedAddressee.Person.SecondGivenNameInitial + ' ';
                    $scope.GreetingName += selectedAddressee.Person.PersonName.FamilyNm;
                }

                if ($scope.selectedAddressee.PartyTypeCd === 'Organization') {
                    $scope.GreetingName += $scope.selectedAddressee.Organization.OrganizationNm;
                }
            }

            if ($scope.AddresseeName2Chk) {
                if ($scope.SelectedAddressee3)
                    $scope.GreetingName += ', ' + $scope.AddresseeName2;
                else
                    $scope.GreetingName += ' and ' + $scope.AddresseeName2;
            }
            if ($scope.SelectedAddressee3 && $scope.Addr3Checkbox) {
                $scope.GreetingName += ' and ' + $scope.AddresseeName3;
            }
        }
    ...
    $scope.$on('PcdwAddrNmOneTwoThreeDrpdwnwGreetNm', function (event) {
        try {
            var AddresseeName = "";
            var AddresseeName3 = "";
