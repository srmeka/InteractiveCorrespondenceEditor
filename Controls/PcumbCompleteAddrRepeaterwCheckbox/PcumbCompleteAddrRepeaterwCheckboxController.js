app.controller('PcumbCompleteAddrRepeaterwCheckboxController', function ($scope, $http, shareData, HomeService) {
    try {
        ...
        $scope.SelectedCompletedAddress = [];
        ...
        $scope.removeCtlRow = function () {
            ...
            if (newItemNo !== 0) {
                ...
                // $scope.SelectedCompletedAddress.pop($scope.TotalCount.length);
                $scope.SelectedCompletedAddress[newItemNo] = undefined;
    ...
    $scope.$on('PcumbCompleteAddrRepeaterwCheckbox', function (event) {
        try {
            ...
            for (var index in $scope.TotalCount) {
                ...
                if ($scope.SelectedCompletedAddress[index]) {
                    AI_value = 'Y';
                    ...
                    HomeService.createSecondaryXMLValue("PROP_DET_INFO", "PROP_INFO_TYPE", "Complete", id);

                }

            }
            HomeService.createPrimaryXML('AI_COMP_ADDR', AI_value);
        }
        catch (ex) {
            $scope.$parent.$parent.error = "We’re sorry, an unexpected error has occurred. Notification has been sent to IT Enterprise Support team";
            HomeService.sendErrorMailandUpdateLog(ex.message, $scope.SelectedDocument.documentFriendlyName);
