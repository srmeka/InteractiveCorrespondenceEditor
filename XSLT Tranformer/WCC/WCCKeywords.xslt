...
>
  ...
  <xsl:variable  name="documentName"  select="/WCC_DATA/RECORD_DELIM/WCC_CLAIM_REC/LTR_NAME/text()"></xsl:variable>
  ...
  <!--<xsl:template match="/">
    ...
    <xsl:variable name="outputXml">
      ...
      <xsl:apply-templates select="msxsl:node-set($xmldoc)/WCC_DATA" mode="removeTags"/>
    </xsl:variable>
