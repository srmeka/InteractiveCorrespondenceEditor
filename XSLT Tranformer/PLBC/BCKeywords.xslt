...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="OB_KEYWORD3">
      <xsl:copy>
        <xsl:if test="$varLob = 'PLA' or $varLob = 'HO' or $varLob = 'DW' or $varLob = 'UMB'">
          ...
          <xsl:if test="$documentName = ('DD-Remove from Direct Debit') or $documentName = ('DD-Waiting for New Funding') or $documentName = ('DD-Remove Multiple Cancel Pymts') or $documentName = ('PL Request Reissue of Check') or $documentName = ('PL BC Blank Letter Template') or $documentName = ('PL Claim Settlement') or $documentName = ('PL Direct Debit Letter') or $documentName = ('PL Not Honored Pymt Request Replace') or $documentName = ('PL Policyholder Acct Statement') or $documentName = ('CMLC  Collection Agency') or $documentName = ('PL Return Pymts') or $documentName = ('Manual Invoice')">
            <xsl:text>OUTGOING</xsl:text>
          </xsl:if>                  
        </xsl:if>

        <xsl:if test="$varLob = 'CA' or $varLob = 'CGL' or $varLob = 'BOP' or $varLob = 'CUMB'">
          <xsl:if test="$documentName = ('PL CCI Letter') or $documentName = ('PL Loss Payee NH Pymt CN DT Stands') or $documentName = ('PL Pymt Accepted CN Stands')">
            <xsl:text>OUTGOING</xsl:text>
          </xsl:if>
          <xsl:if test="$documentName = ('DD-Remove from Direct Debit') or $documentName = ('DD-Waiting for New Funding') or $documentName = ('DD-Remove Multiple Cancel Pymts') or $documentName = ('PL Request Reissue of Check') or $documentName = ('PL BC Blank Letter Template') or $documentName = ('PL Claim Settlement') or $documentName = ('PL Direct Debit Letter') or $documentName = ('PL Not Honored Pymt Request Replace') or $documentName = ('PL Policyholder Acct Statement') or $documentName = ('CMLC  Collection Agency') or $documentName = ('Manual Invoice')">
            <xsl:text>OUTGOING</xsl:text>
          </xsl:if>          
        </xsl:if>
  ...
  <!-- Cancel Mail Date -->
    <xsl:template match="OB_KEYWORD7">
      <xsl:copy>
        <xsl:choose>
          <xsl:when test="$varLob = ('PLA') or $varLob = ('HO') or $varLob = ('DW')  or $varLob = ('UMB')">
            <xsl:if test="$documentName = ('PL Not Honored Pymt on Pending CN') or $documentName = ('PL Partial Pymt Pending CN Stands') or $documentName = ('PL Not Honored Pymt Orig CN DT Stnd') or $documentName = ('PL Not Honored Pymt Orig CN DT Stnd') or $documentName = ('PL Return Pymt Letter')">
              <xsl:if test="/CUSTOMER_DATA/RECORD_DELIM/BILLING_REC/OB_KEYWORD7 != ''">
              <xsl:value-of select="substring(/CUSTOMER_DATA/RECORD_DELIM/BILLING_REC/OB_KEYWORD7,6,2)"/>/<xsl:value-of select="substring(/CUSTOMER_DATA/RECORD_DELIM/BILLING_REC/OB_KEYWORD7,9,2)"/>/<xsl:value-of select="substring(/CUSTOMER_DATA/RECORD_DELIM/BILLING_REC/OB_KEYWORD7,0,5)"/>
              </xsl:if>
              <xsl:if test="/CUSTOMER_DATA/RECORD_DELIM/BILLING_REC/OB_KEYWORD7 = ''">
                <xsl:text> </xsl:text>
              </xsl:if>           
    ...
    <xsl:template match="OB_KEYWORD8">
      <xsl:copy>
        ...
        <xsl:if test="$varLob = 'CA' or $varLob = 'GL' or $varLob = 'BOP' or $varLob = 'CUMB'">
          <xsl:text></xsl:text>
        </xsl:if>
      </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="OB_KEYWORD9">
      <xsl:copy>
        <xsl:if test="$varLob = 'CA' or $varLob = 'GL' or $varLob = 'BOP' or $varLob = 'CUMB'">
          <xsl:if test="$documentName = ('PL Not Honored Pymt on Pending CN') or $documentName = ('PL Partial Pymt Pending CN Stands') or $documentName = ('PL Not Honored Pymt Orig CN DT Stnd') or $documentName = ('PL Return Pymts') or $documentName = ('PL Return Pymt Letter')">
    ...
    <xsl:template match="OB_KEYWORD10">
      <xsl:copy>
        <xsl:if test="$varLob = 'CA' or $varLob = 'GL' or $varLob = 'BOP' or $varLob = 'CUMB'">
          ...
          <xsl:if test="$documentName = ('PL Not Honored Pymt Orig CN DT Stnd') or $documentName = ('PL Return Pymts')">
            <xsl:text>NOT HONORED PYMT CANCEL DT STANDS</xsl:text>
          </xsl:if>
    ...
    <xsl:template match="OUTBND_DOCTYPE">
      <xsl:copy>
        <xsl:choose>
          <xsl:when test="$documentName = ('PL Not Honored Pymt on Pending CN') or $documentName = ('PL Not Honored Pymt Orig CN DT Stnd') or $documentName = ('PL Return Pymt Letter') or $documentName = ('PL Pymt Accepted CN Stands')">
            <xsl:if test="$varLob = 'CA' or $varLob = 'GL' or $varLob = 'BOP' or $varLob = 'CUMB'">
           ...
           <!-- INCIDENT FIX  - 81640 -->
          <xsl:when test="$documentName = ('PL Return Pymts')">
            <xsl:if test="$varLob = 'CA' or $varLob = 'GL' or $varLob = 'BOP' or $varLob = 'CUMB'">
  ...
  <xsl:template match="CNC_EFF_DT">
    <xsl:copy>
      <xsl:choose>
        <xsl:when test="$documentName = 'PL Not Honored Pymt on Pending CN'">
